...
if command -v apt-get >/dev/null; then
  ...
  DEBIAN_FRONTEND=noninteractive apt-get install -y \
    ...
    restic openssh-client openssl apache2-utils smartmontools curl ca-certificates nfs-common cifs-utils
elif command -v dnf >/dev/null; then
  dnf install -y python3 python3-pip python3-setuptools python3-wheel python3-pyyaml python3-boto3 \
    restic openssh-clients openssl httpd-tools smartmontools curl ca-certificates nfs-utils cifs-utils
elif command -v yum >/dev/null; then
  yum install -y python3 python3-pip python3-setuptools python3-wheel python3-pyyaml python3-boto3 \
    restic openssh-clients openssl httpd-tools smartmontools curl ca-certificates nfs-utils cifs-utils
else
  echo "No supported package manager detected. Install Python 3.10+, PyYAML, boto3, restic, OpenSSH client, OpenSSL, htpasswd and smartmontools manually." >&2
  exit 1
fi
...
The top-level ./scripts/install.sh installer can install the pinned upstream rest-server binary only after SHA-256 verification.
Next:
  1. Mount your dedicated RAID/ZFS/XFS/ext4/NFS storage at the selected root if applicable.
  2. Prefer: sudo ./scripts/install.sh --role all --repo-root "$ROOT" (downloads only the pinned/SHA-verified rest-server when needed).
