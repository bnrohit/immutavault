...
if [[ $SKIP_PACKAGES -eq 0 ]]; then ./scripts/install_appliance.sh "$REPO_ROOT"; else ./scripts/install_controller.sh "$REPO_ROOT"; fi

RESTIC_OK=0
if command -v restic >/dev/null 2>&1 && ./scripts/check_restic.sh "$(command -v restic)" >/dev/null 2>&1; then RESTIC_OK=1; fi
if [[ $RESTIC_OK -eq 0 && $INSTALL_RESTIC -eq 1 ]]; then ./scripts/install_restic.sh; RESTIC_OK=1; fi
if [[ $RESTIC_OK -eq 0 ]]; then
  echo "A compatible restic >= 0.19.1 is required. No services were enabled." >&2; exit 3
fi

if [[ "$ROLE" == repository || "$ROLE" == all ]]; then
  REST_SERVER_OK=0
  if command -v rest-server >/dev/null 2>&1 && ./scripts/check_rest_server.sh "$(command -v rest-server)" >/dev/null 2>&1; then REST_SERVER_OK=1; fi
  if [[ $REST_SERVER_OK -eq 0 && $INSTALL_REST_SERVER -eq 1 ]]; then ./scripts/install_rest_server.sh; REST_SERVER_OK=1; fi
  if [[ $REST_SERVER_OK -eq 0 ]]; then
    echo "A compatible rest-server >= 0.14.0 is required. No repository service was enabled." >&2; exit 3
  fi
...
if [[ $ENABLE -eq 1 ]]; then
  if command -v rest-server >/dev/null 2>&1 && [[ "$ROLE" != controller ]]; then systemctl enable --now immutavault-rest-server.service; fi
  if [[ "$ROLE" == all || "$ROLE" == controller ]]; then
    ...
    # The legacy broad backup timer remains available for existing deployments.
    # Saving the first scheduled named policy disables it to prevent duplicate runs.
    systemctl enable --now immutavault-backup.timer
  fi
  if [[ "$ROLE" == all ]]; then systemctl enable --now immutavault-retention.timer immutavault-verify.timer; fi
  # DR failover timers remain opt-in after controlled recovery acceptance.
fi
